                  ...
                  <tbody>
                     @foreach($product as $product)
                      <th class="pl-0 border-light" scope="row">
                        <div class="media align-items-center"><a class="reset-anchor d-block animsition-link" href="detail.html"><img src="{{asset('ProductImages/' .$product['item']['image_path'])}}" alt="..." width="70"/></a>
                          <div class="media-body ml-3"><strong class="h6"><a class="reset-anchor animsition-link" href="detail.html">{{$product['item']['name']}}</a></strong></div>
                        </div>
                      </th>
                      <td class="align-middle border-light">
                        <p class="mb-0 small">{{$product['item']['sale_price']}}</p>
                      </td>
                      <td class="align-middle border-light">
                        <div class="border d-flex align-items-center justify-content-between px-3"><span class="small text-uppercase text-gray headings-font-family">{{$product['quantity']}}</span>
                          <div class="quantity">
                            <button class="dec-btn p-0"><i class="fas fa-caret-left"></i></button>
                      ...
                      <td class="align-middle border-light">
                        <p class="mb-0 small">{{($product['item']['sale_price'])*($product['quantity'])}}</p>
                      </td>
                      <td class="align-middle border-light">
                        
<form action="{{route('deleteCart', $product['item']['id'])}}" method="POST">
    @csrf
    @method('delete')
    <button type="submit" class="btn-white"><i class="fas fa-trash-alt small text-muted"></i></button>
</form>
                      </td>
                    </tr>
                      @endforeach
